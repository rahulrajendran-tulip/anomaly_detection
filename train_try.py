...
class AnomalyClassifier(LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        auroc = self.val_auroc.update(pred, target)
...
def get_basic_callbacks() -> list:
    ...
    ckpt_callback_acc = ModelCheckpoint(
        filename="epoch{epoch:03d}-{'val/acc': .4f}",
        monitor="val/acc",
    ...
    ckpt_callback_f1 = ModelCheckpoint(
        filename="epoch{epoch:03d}-{'val/f1': .4f}",
        monitor="val/f1",
        auto_insert_metric_name=False,
        save_top_k=3,
        mode="max",
    ...
    ckpt_callback_auroc = ModelCheckpoint(
        filename="epoch{epoch:03d}-auroc{'val/auroc': .4f}",
        monitor="val/auroc",
        auto_insert_metric_name=False,
        save_top_k=3,
        mode="max",
    ...
    return [ckpt_callback_acc, ckpt_callback_f1, ckpt_callback_auroc, lr_callback]
